-- Fix: allow invite_token to be nullable before redaction, then move to hashed storage

-- 1) Add hashed token column and index (idempotent)
...
-- 2) Backfill hashes for existing tokens
...
-- 3) Make plaintext column nullable, then redact it
ALTER TABLE public.gallery_invites
...
-- 4) Update create_gallery_invite to store only hashed tokens and return the raw token once
...
-- 5) Update validate_gallery_invite to verify via hash (with legacy fallback)
...
-- 6) Optional: create a safe view that never exposes token fields
