-- Phase 1: Fix Authentication & Authorization - Database Security

-- First, let's fix the RLS policies to remove anonymous access where it's not needed
-- and ensure proper user isolation
...
-- Update galleries policies for better security
-- Only allow authenticated photographers to see their own galleries
-- Public galleries will be accessed through a separate secure mechanism
...
-- Update images policies - only accessible through authenticated sessions or gallery access tokens
...
-- Update image variants policies
...
-- Update favorites to work with gallery access sessions
-- Remove the overly permissive policy
CREATE POLICY "Gallery sessions can manage favorites" 
ON public.favorites 
FOR ALL
USING (
  ...
  -- Or if there's a valid gallery access session (will be checked by application logic)
);

-- Create function to verify gallery access via session token
...
-- Clean up expired sessions periodically (can be called by a cron job)
