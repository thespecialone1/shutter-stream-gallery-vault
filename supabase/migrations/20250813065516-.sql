...
-- Replace with a safe sections policy limited to public galleries
CREATE POLICY IF NOT EXISTS "Public can view sections from public galleries" 
ON public.sections FOR SELECT
USING (
  EXISTS (
    SELECT 1 FROM public.galleries g
    WHERE g.id = sections.gallery_id AND g.is_public = true
  )
);
