...
DECLARE
  ...
  _attempt_type text := attempt_type;
BEGIN
  -- Check if currently blocked
  SELECT EXISTS (
    SELECT 1 FROM public.auth_rate_limits
    WHERE auth_rate_limits.identifier = _identifier
    AND auth_rate_limits.attempt_type = _attempt_type
  ...
  IF current_attempts >= max_attempts THEN
    INSERT INTO public.auth_rate_limits (
      ...
      _identifier, _attempt_type, 1, now() + interval '1 hour'
    )
