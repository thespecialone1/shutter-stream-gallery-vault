-- Fix create_session_from_share_link function to properly validate gallery passwords
CREATE OR REPLACE FUNCTION public.create_session_from_share_link(
  invite_token text DEFAULT NULL::text, 
  alias text DEFAULT NULL::text, 
  client_ip inet DEFAULT NULL::inet, 
  user_agent text DEFAULT NULL::text, 
  gallery_password text DEFAULT NULL::text
)
RETURNS json
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  IF gallery_record.password_hash IS NOT NULL THEN
    ...
    -- Validate password using verify_password function
